...
def transform(dfs, dw=None, dw_sample=None, verbose=False):
    ...
    df_grandegrupo = dfs['stg_cbo2002_grande_grupo'].reset_index(drop=True)
    df_subgrupoprinc = dfs['stg_cbo2002_subgrupo_principal'].reset_index(drop=True)
    df_subgrupo = dfs['stg_cbo2002_subgrupo'].reset_index(drop=True)
    df_familia = dfs['stg_cbo2002_familia'].reset_index(drop=True)
    df = dfs['stg_cbo2002_ocupacao'].reset_index(drop=True)

    # classifications codes 
    df['grande_grupo'] = df['ocupacao'].apply(lambda x: x[:1])
    df['subgrupo_principal'] = df['ocupacao'].apply(lambda x: x[:2])
    df['subgrupo'] = df['ocupacao'].apply(lambda x: x[:3])
    df['familia'] = df['ocupacao'].apply(lambda x: x[:4])

    # grande_grupo_desc
    df_grandegrupo['grande_grupo'] = df_grandegrupo['grande_grupo'].apply(str)
    df = pd.merge(df , df_grandegrupo, how='left', on='grande_grupo')

    # subgrupo_princial_descricao
    df = pd.merge(df , df_subgrupoprinc, how='left', on='subgrupo_principal')

    # subgrupo
    df = pd.merge(df , df_subgrupo, how='left', on='subgrupo')

    # familia
    df = pd.merge(df , df_familia, how='left', on='familia')

    # Set surrogate keys
    df.reset_index(inplace=True, drop=True)
    df['cbo2002_sk'] = df.index + 1

    # Select and order columns
    df = df[[
        'cbo2002_sk', 'ocupacao', 'ocupacao_descricao', 'grande_grupo',
        'subgrupo_principal', 'subgrupo', 'familia',
        'grande_grupo_descricao', 'subgrupo_principal_descricao',
        'subgrupo_descricao', 'familia_descricao',
    ]]
...
def get_division_df():
    # http://pdet.mte.gov.br/o-programa/tabelas-de-classificacao/nivel-setorial/divisao-de-atividade-economica-cnae-2-0
    data = {
    ...
    return pd.DataFrame(data.items(), columns=['cod_divisao', 'divisao'])

def get_group_df():
    # http://pdet.mte.gov.br/o-programa/tabelas-de-classificacao/nivel-setorial/grupo-de-atividade-economica-cnae-2-0
    data = {
    #Código	Categoria	Descrição',
    ...
    '20.6':'Fabricação de sabões, detergentes, produtos de limpeza, cosméticos, produtos de perfuma',
    ...
    '23.3':'Fabricação de artefatos de concreto, cimento, fibrocimento, gesso e materiais semelhant',
    ...
    '26.4':'Fabricação de aparelhos de recepção, reprodução, gravação e amplificação de áudio e víd',
    '26.5':'Fabricação de aparelhos e instrumentos de medida, teste e controle; cronômetros e relóg',
    ...
    '32.5':'Fabricação de instrumentos e materiais para uso médico e odontológico e de artigos ópti',
    ...
    '42.2':'Obras de infra-estrutura para energia elétrica, telecomunicações, água, esgoto e transp',
    ...
    '46.1':'Representantes comerciais e agentes do comércio, exceto de veículos automotores e motoc',
    ...
    '46.5':'Comércio atacadista de equipamentos e produtos de tecnologias de informação e comunicaç',
    '46.6':'Comércio atacadista de máquinas, aparelhos e equipamentos, exceto de tecnologias de inf',
    '46.7':'Comércio atacadista de madeira, ferragens, ferramentas, material elétrico e material de',
    ...
    '47.5':'Comércio varejista de equipamentos de informática e comunicação; equipamentos e artigos',
    '47.6':'Comércio varejista de artigos culturais, recreativos e esportivos',
    '47.7':'Comércio varejista de produtos farmacêuticos, perfumaria e cosméticos e artigos médicos',
    '47.8':'Comércio varejista de produtos novos não especificados anteriormente e de produtos usad',
    ...
    '87.1':'Atividades de assistência a idosos, deficientes físicos, imunodeprimidos e convalescent',
    '87.2':'Atividades de assistência psicossocial e à saúde a portadores de distúrbios psíquicos,',
    ...
    return pd.DataFrame(data.items(), columns=['cod_grupo', 'grupo'])
...
def get_class_df():
    data = {
        ...
        '17.41-9':'Fabricação de produtos de papel, cartolina, papel-cartão e papelão ondulado para uso industrial, comercial e de escritório',
        ...
        '47.29-6':'Comércio varejista de produtos alimentícios em geral ou especializado em produtos alimentícios não especificados anteriormente; produ',
        ...
        '47.57-1':'Comércio varejista especializado de peças e acessórios para aparelhos eletroeletrônicos para uso doméstico, exceto informática e comu',
        ...
        '49.29-9':'Transporte rodoviário coletivo de passageiros, sob regime de fretamento, e outros transportes rodoviários não especificados anteriorm',
        ...
        '87.11-5':'Atividades de assistência a idosos, deficientes físicos, imunodeprimidos e convalescentes prestadas em residências coletivas e partic',
    ...
    return pd.DataFrame(data.items(), columns=['cod_classe', 'classe'])
